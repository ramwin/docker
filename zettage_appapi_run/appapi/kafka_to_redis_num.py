...
kafka_consumer = KafkaConsumer(NUMBER_TOPIC)
def kafka_to_redis_number():
    global redis_connector
    global kafka_consumer
    while True:
        try:
            for msg in kafka_consumer:
                ...
                print(msg)
                print(msg[4])
                redis_connector.rpush(REDIS_NUMBER_LIST,msg[4])
        except:
            try:
                redis_connector = redis.StrictRedis(host=REDIS_HOST,port=REDIS_PORT,db=REDIS_DB)
                kafka_consumer = KafkaConsumer(NUMBER_TOPIC)
